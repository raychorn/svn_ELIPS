package com.utils {
	...
	public class StringUtils {
		...
		private static var __RegExp__:Object = {
			'isUrl': /^http(s)?:\/\/((\d+\.\d+\.\d+\.\d+)|(([\w-]+\.)+([a-z,A-Z][\w-]*)))(:[1-9][0-9]*)?(\/([\w-.\/:%+@&=]+[\w- .\/?:%+@&=]*)?)?(#(.*))?$/i,
			'isUserName': /^[\w'_]+\s?[\w'_]+$/i,
			'isEmailAddress': /[a-z0-9!#$%&'*+\/=?^_`{|}~-]+(?:\.[a-z0-9!#$%&'*+\/=?^_`{|}~-]+)*@(?:[a-z0-9](?:[a-z0-9-]*[a-z0-9])?\.)+[a-z0-9](?:[a-z0-9-]*[a-z0-9])?/i,
			'isPassword': /^(?=[\-_a-zA-Z0-9]*?[A-Z])(?=[\-_a-zA-Z0-9]*?[a-z])(?=[\-_a-zA-Z0-9]*?[0-9])[\-_a-zA-Z0-9]{6,}$/i
		};
		...
		private static function __is__(pattern:RegExp,target:String):Boolean {
			return (pattern.exec(target));
		}
		
		public static function isUrl(url:String):Boolean {
			return StringUtils.__is__(StringUtils.__RegExp__['isUrl'],url);
		}
		
		public static function isUserName(name:String):Boolean {
			return StringUtils.__is__(StringUtils.__RegExp__['isUserName'],name);
		}

		public static function isEmailAddress(email:String):Boolean {
			return StringUtils.__is__(StringUtils.__RegExp__['isEmailAddress'],email);
		}
		
		public static function isPassword(password:String):Boolean {
			return StringUtils.__is__(StringUtils.__RegExp__['isPassword'],password);
		}
